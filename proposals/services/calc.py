...
3. Per proposal:
     ...
     total_rig_days        = MAX(total_days of all sections) + mob_days + demob_days
         ↑ mirrors the Excel formula `=MAX(Chart!CA212:CA232)` — parallel operations
           mean the rig commitment equals the longest section, not the sum.
"""
...
def calculate_proposal_totals(proposal) -> ProposalTotals:
    ...
    for section in proposal.casing_sections.all():
        ...
        if section.total_days > max_section_days:
            max_section_days = section.total_days

    rig_days = max_section_days + Decimal(proposal.mob_days or 0) + Decimal(proposal.demob_days or 0)
