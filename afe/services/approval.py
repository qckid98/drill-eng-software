...
All transitions use select_for_update() to prevent race conditions
...
def _log(afe: AFE, actor, action: str, from_status: str, to_status: str, comment: str = "") -> None:
    ...
    logger.info(
        "AFE Approval: %s %s -> %s by %s (afe=%s)",
        action, from_status, to_status, actor.username, afe.pk,
    )


def submit(afe: AFE, actor, comment: str = "") -> None:
    with transaction.atomic():
        afe = AFE.objects.select_for_update().get(pk=afe.pk)
